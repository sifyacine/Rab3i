...
- Staff accounts are created from the dashboard via `usersService.createUser`, which signs up through a **standalone non-persistent Supabase client** (`createStandaloneAuthClient` in `src/lib/supabase.ts`) so the manager's own session is not replaced, then upserts the `profiles` row with the chosen role.
